package cn.cosx.blog.mentor.agent.agent.websearch;

import cn.cosx.blog.mentor.agent.agent.BaseAgent;
import cn.cosx.blog.mentor.agent.entity.record.AgentState;
import cn.cosx.blog.mentor.agent.entity.record.RoundMode;
import cn.cosx.blog.mentor.agent.entity.record.RoundState;
import cn.cosx.blog.mentor.agent.entity.record.SearchResult;
import cn.cosx.blog.mentor.agent.prompts.ReactAgentPrompts;
...
public class WebSearchReactAgent extends BaseAgent {
    ...
    private Flux<String> streamInternal(String conversationId, String question) {
        ...
        initTimers();
        clearUsedTools();
        // unicast：创建一个单播的 Sink，只允许一个下游消费者消费，使用 onBackpressureBuffer 进行背压缓冲，将来不及消费的信息缓冲起来
